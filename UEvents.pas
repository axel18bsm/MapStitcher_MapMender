...
//    E                Passer en mode selection du rectangle export
//    Echap            Annuler le mode export
...
begin
  ...
  begin
    ...
    // E : entrer en mode selection du rectangle d export
  ...
  // -----------------------------------------------------------
  //  Mode selection du rectangle d export
  //  1er clic = coin haut-gauche (P1)
  //  2eme clic = coin bas-droit  (P2) → sauvegarde immediate
  // -----------------------------------------------------------
  else if AppState = asExportSelect then
  begin
    // Mise a jour continue pour le rubber-band
    ExportMousePos := ScreenToWorld(MousePos);

    if IsMouseButtonPressed(MOUSE_BUTTON_LEFT) then
    begin
      if not ExportP1Set then
      begin
        // Poser le premier coin
        ExportP1    := ScreenToWorld(MousePos);
        ExportP1Set := True;
      end
      else
      begin
        // Poser le second coin et lancer la sauvegarde
        ...
        SavedMessage  := 'Fusionne et sauvegarde : ' + ExtractFileName(SavePath);
        SavedMsgTimer := 5.0;    // message visible 5 secondes

        AppState := asEditing;   // revenir en edition apres sauvegarde
      end;
    end;

    // Echap : annuler sans sauvegarder
...
//  DrawExportRect
//  Rubber-band du rectangle d export.
//  - Avant P2 : dessine de P1 au curseur courant
//  - Affiche les dimensions en pixels monde
// ============================================================
procedure DrawExportRect;
var
  SP1, SP2 : TVector2;
  SR       : TRectangle;
  FillCol  : TColor;
  ...
  DimStr   : String;
begin
  if not ExportP1Set then Exit;

  // Convertir les deux extremites en coordonnees ecran
  SP1 := WorldToScreen(ExportP1);

  // Si P2 n est pas encore pose, suivre la souris
  if ExportP2Set then
    SP2 := WorldToScreen(ExportP2)
  else
    SP2 := WorldToScreen(ExportMousePos);

  // Rectangle normalise (gere les clics de bas-droite vers haut-gauche)
  SR := MakeRect(Min(SP1.x, SP2.x),
                 Min(SP1.y, SP2.y),
                 Abs(SP2.x - SP1.x),
                 Abs(SP2.y - SP1.y));

  // Fond jaune tres transparent
  FillCol := MakeColor(255, 240, 0, 22);
  DrawRectangle(Round(SR.x), Round(SR.y),
                Round(SR.width), Round(SR.height), FillCol);

  // Bordure jaune
  LineCol := MakeColor(255, 220, 0, 200);
  DrawRectangleLinesEx(SR, 2, LineCol);

  // Marqueur du coin P1
  DrawCircle(Round(SP1.x), Round(SP1.y), 5, YELLOW);

  // Dimensions en pixels monde (affichees au-dessus du rectangle)
  if not ExportP2Set then
    WP2 := ExportMousePos
  else
    WP2 := ExportP2;

  DimStr := Format('%d x %d px',
                   [Round(Abs(WP2.x - ExportP1.x)),
                    Round(Abs(WP2.y - ExportP1.y))]);
  DrawText(PChar(DimStr),
           Round(SR.x) + 6,
           Round(SR.y) - 22, 14, YELLOW);
end;
...
begin
  ...
  // --- Rubber-band du rectangle d export ---
  if AppState = asExportSelect then
    DrawExportRect;
end;
...
begin
  ...
  case AppState of
    ...
    asExportSelect : ModeStr := 'MODE : EXPORT';
  ...
  // ---  Instruction en mode export (bandeau en haut) ---
  if AppState = asExportSelect then
  begin
    DrawRectangle(ScreenWidth div 2 - 310, 8, 620, 34,
                  MakeColor(60, 45, 5, 220));
    DrawRectangleLines(ScreenWidth div 2 - 310, 8, 620, 34, YELLOW);

    if not ExportP1Set then
      DrawText('Cliquez pour definir le coin HAUT-GAUCHE du rectangle',
               ScreenWidth div 2 - 290, 16, 15, YELLOW)
    else
      DrawText('Cliquez pour definir le coin BAS-DROIT du rectangle',
               ScreenWidth div 2 - 278, 16, 15, YELLOW);
  end;

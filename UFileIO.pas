...
procedure SaveMergedImage(const ADestPath: String);
const
  PRIORITY : array[0..2] of TMapRole = (mrCenter, mrLeft, mrRight);
var
  OutW, OutH   : Integer;
  OutImg       : TImage;
  X, Y         : Integer;
  ...
  R            : TMapRole;
begin
  OutW := Round(Abs(ExportP2.x - ExportP1.x));
  OutH := Round(Abs(ExportP2.y - ExportP1.y));

  if (OutW <= 0) or (OutH <= 0) then Exit;

  // Image de sortie initialisee en blanc
  OutImg := GenImageColor(OutW, OutH, WHITE);

  for Y := 0 to OutH - 1 do
  begin
    WY := Round(ExportP1.y) + Y;

    for X := 0 to OutW - 1 do
    begin
      WX    := Round(ExportP1.x) + X;
      Found := False;
      ...
      begin
        ...
        begin
          PixColor := GetImageColor(Maps[R].Image, MapX, MapY);
          ImageDrawPixel(@OutImg, X, Y, PixColor);
          Found := True;
          Break;
        end;
      end;
      ...
      if not Found then
        ImageDrawPixel(@OutImg, X, Y, WHITE);
    end;
  end;
